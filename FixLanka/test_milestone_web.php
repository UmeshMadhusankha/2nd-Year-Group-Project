<?php
// filepath: c:\xampp\htdocs\2nd-Year-Group-Project\FixLanka\test_milestone_web.php

require_once 'config/database.php'; // Correct path relative to web root
require_once 'models/ContractModel.php';

// Simulate interactions manually by setting session and calling model directly
// (Since we can't easily curl localhost from inside php unless simple)

echo "<h1>Milestone Workflow Test</h1>";
echo "<pre>";
...
try {
    ...
    // Create Dummy Customer (User table)
    $pdo->exec("INSERT INTO user (user_id, f_name, l_name, email, password) VALUES (9991, 'Test', 'Customer', 'testcust@example.com', 'pass')");
    
    // Create Dummy Company (Company table - distinct from User)
    // Company has its own password and email
    try {
    ...
    if ($m['status'] === 'completed') echo "   [PASS] Status is completed<br>"; else echo "   [FAIL] Status should be completed<br>";
