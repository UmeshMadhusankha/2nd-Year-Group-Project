<?php
// filepath: c:\xampp\htdocs\2nd-Year-Group-Project\FixLanka\test_escrow_flow_v2.php

require_once 'config/database.php';
require_once 'models/ContractModel.php';
require_once 'models/EscrowModel.php';

echo "<h1>Escrow Flow Test V2 - VERSION 3</h1>";
echo "<pre>";
...
try {
    ...
    $pdo->exec("DELETE FROM user WHERE user_id IN (8881, 8882)");
    
    // Create Users
    $pdo->exec("INSERT INTO user (user_id, f_name, l_name, email, password) VALUES (8881, 'Escrow', 'Cust', 'ecust@test.com', 'pass')");
    
    // Create Company (Linked to its own ID 8882, disjoint from User 8881)
    ...
    // Using new signature: deposit($ownerId, $amount, $ownerType='user', ...)
    if ($escrowModel->deposit(8881, 1000.00, 'user', 'Initial Deposit')) echo "[PASS]<br>"; else echo "[FAIL]<br>";
    
    $w = $escrowModel->getWallet(8881, 'user');
    ...
    $wc = $escrowModel->getWallet(8882, 'company');
