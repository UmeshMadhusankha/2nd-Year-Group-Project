...
?>
<link rel="stylesheet" href="<?= $basePath ?>/assets/css/admin & moderator/issues.css">
...
async function loadIssues(page = 1) {
    ...
    try {
        ...
        displayIssues(result.data, result.pagination);
...
async function resolveIssue(id) {
    ...
    try {
        ...
        console.error('[v0] Error loading issue details:', error);
...
document.getElementById('resolveForm').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`<?= $basePath ?>/api/reports/${currentIssueId}`, {
            method: 'PUT',
            headers: {
