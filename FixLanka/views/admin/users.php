...
?>
<link rel="stylesheet" href="<?= $basePath ?>/assets/css/admin & moderator/users.css">
...
<script>
    ...
    async function editUser(userId) {
        try {
            const response = await fetch(`<?= $basePath ?>/api/accounts/${userId}`);
            const data = await response.json();
    ...
    async function confirmDelete() {
        ...
        try {
            const response = await fetch(`<?= $basePath ?>/api/accounts/${deleteUserId}`, {
                method: 'DELETE'
            });
