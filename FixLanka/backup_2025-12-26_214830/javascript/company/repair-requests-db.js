...
async function loadCompanyDefaults() {
    try {
        console.log('Loading company defaults for user_id:', currentCompanyId);
        ...
        if (result.success && result.data) {
            ...
            console.log('Company defaults loaded:', companyDefaults);
        }
    } catch (error) {
...
function loadDirectRequests() {
    ...
    console.log('Direct requests feature: Coming soon');
}
        ...
        <tr data-request-id="${request.request_id}">
            <td>
                <div>
                    ...
                    <p style="margin: 0; color: var(--text-secondary); font-size: var(--font-size-sm);">
                        #REQ-${request.request_id} • ${escapeHtml(request.category_name || 'General')}
                    </p>
                </div>
            </td>
            <td>
