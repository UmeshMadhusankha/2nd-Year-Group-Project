...
-- Check if columns exist before updating to avoid errors on fresh run (Though ALTER adds them)
...
-- ALTER TABLE `companyquotation` ADD INDEX `idx_schedule_type` (`work_schedule_type`);
-- Commented out index creation to avoid error if it already exists or if key length issues arise
