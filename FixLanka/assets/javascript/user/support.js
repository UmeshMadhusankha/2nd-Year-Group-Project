// Support Page JavaScript

document.addEventListener('DOMContentLoaded', function() {
    // Add smooth scroll for home button
    const homeBtn = document.querySelector('.btn-home');
    if (homeBtn) {
        homeBtn.addEventListener('click', function(e) {
            // Smooth transition when going to home
            window.location.href = this.getAttribute('href');
        });
    }

    // Add animation on scroll for cards
    const observerOptions = {
        threshold: 0.1,
        rootMargin: '0px 0px -50px 0px'
    };

    const observer = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.style.opacity = '0';
                entry.target.style.transform = 'translateY(20px)';
                
                setTimeout(() => {
                    entry.target.style.transition = 'all 0.5s ease';
                    entry.target.style.opacity = '1';
                    entry.target.style.transform = 'translateY(0)';
                }, 100);
                
                observer.unobserve(entry.target);
            }
        });
    }, observerOptions);

    // Observe quick link cards
    const quickLinkCards = document.querySelectorAll('.quick-link-card');
    quickLinkCards.forEach((card, index) => {
        card.style.transitionDelay = `${index * 0.1}s`;
        observer.observe(card);
    });

    // Observe contact methods
    const contactMethods = document.querySelectorAll('.contact-method');
    contactMethods.forEach((method, index) => {
        method.style.transitionDelay = `${index * 0.1}s`;
        observer.observe(method);
    });

    // Observe topic cards
    const topicCards = document.querySelectorAll('.topic-card');
    topicCards.forEach((card, index) => {
        card.style.transitionDelay = `${index * 0.1}s`;
        observer.observe(card);
    });

    // Emergency banner entrance animation
    const emergencyBanner = document.querySelector('.emergency-banner');
    if (emergencyBanner) {
        observer.observe(emergencyBanner);
    }

    // Add click tracking for contact methods (for analytics)
    const emailMethod = document.querySelector('.method-icon.email');
    const phoneMethod = document.querySelector('.method-icon.phone');
    
    if (emailMethod) {
        emailMethod.addEventListener('click', function() {
            // Could be used for analytics tracking
            console.log('Email contact method clicked');
        });
    }

    if (phoneMethod) {
        phoneMethod.addEventListener('click', function() {
            // Could be used for analytics tracking
            console.log('Phone contact method clicked');
        });
    }
});
